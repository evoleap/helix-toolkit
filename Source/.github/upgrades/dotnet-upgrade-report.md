...
|:----------------------------------------------------------------|:---------------------------------------------:|:--------------------:|:---------------------------|
| HelixToolkit\HelixToolkit.csproj               | .NETPortable,Version=v4.0,Profile=Profile136  | net8.0      | d016fa68, 7a0fa38c, [fix]  |
...
| **[NEW]** | **Fix HelixToolkit.csproj conversion to SDK-style format - removed hybrid PCL/SDK conflicts**   |
...
- **[POST-UPGRADE FIX]** Cleaned up hybrid PCL/SDK-style format that prevented Visual Studio from loading the project properly
- Enabled assembly signing with key file specification
- Moved assembly metadata (AssemblyTitle, Description, AssemblyVersion, Company, Copyright) from GlobalAssemblyInfo.cs to project file
- Deleted AssemblyInfo.cs (now handled by SDK)
- Removed all legacy PCL-specific elements (TargetFrameworkProfile, ProjectTypeGuids, Microsoft.Portable.CSharp.targets)
- Configured proper documentation file generation
...
### Issue: Projects not appearing in Configuration Manager / Build skipped
**Problem:** After initial upgrade, HelixToolkit.csproj had a hybrid PCL/SDK-style format that Visual Studio couldn't load properly. The project would appear in Solution Explorer but not in Configuration Manager, and builds would be skipped.

**Cause:** The project file contained both legacy Portable Class Library elements (TargetFrameworkProfile, ProjectTypeGuids) and new SDK-style elements (TargetFramework=net8.0), creating an invalid hybrid state.

**Fix:** Completely converted project to clean SDK-style format, removing all PCL-specific elements and properly structuring it using `<Project Sdk="Microsoft.NET.Sdk">`.
...
- ? **COMPLETED:** Reload solution in Visual Studio - projects now appear in Configuration Manager
- ? **COMPLETED:** Build solution - all projects compile successfully
- Test the solution functionality to ensure all features work correctly
...
- Run WPF applications to verify UI and functionality work as expected on .NET 8.0
